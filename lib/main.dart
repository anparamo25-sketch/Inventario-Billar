...
class Product{final String name;final int price,packSize;final String presentation,provider;int warehouse,freezer;Product(this.name,this.price,this.packSize,{this.presentation='unidad',this.provider='',this.warehouse=0,this.freezer=0});Map<String,dynamic> j()=>{'name':name,'warehouse':warehouse,'freezer':freezer};}
...
class Day{final DateTime date;final Map<String,int> opening,finalStock,sold,refill;final List<Loan> loans;final List<Consumption> consumptions;final List<Movement> movements;final List<Map<String,dynamic>> expenseDetails;final int sales,expenses,expected,actual,difference;Day({required this.date,required this.opening,required this.finalStock,required this.sold,required this.refill,required this.loans,required this.consumptions,required this.movements,this.expenseDetails=const [],required this.sales,required this.expenses,required this.expected,required this.actual,required this.difference});Map<String,dynamic> j()=>{'date':date.toIso8601String(),'opening':opening,'final':finalStock,'sold':sold,'refill':refill,'loans':loans.map((x)=>x.j()).toList(),'consumptions':consumptions.map((x)=>x.j()).toList(),'movements':movements.map((x)=>x.j()).toList(),'expenseDetails':expenseDetails,'sales':sales,'expenses':expenses,'expected':expected,'actual':actual,'difference':difference};static Map<String,int> mp(dynamic v){final m=Map<String,dynamic>.from(v??{});return m.map((k,v)=>MapEntry(k,(v as num).toInt()));}factory Day.from(Map<String,dynamic> x)=>Day(date:DateTime.tryParse(x['date']??'')??DateTime.now(),opening:mp(x['opening']),finalStock:mp(x['final']),sold:mp(x['sold']),refill:mp(x['refill']),loans:(x['loans'] as List? ?? []).map((v)=>Loan((v['person']??'') as String,(v['amount'] as num? ?? 0).toInt(),(v['reason']??'') as String,DateTime.tryParse(v['date']??'')??DateTime.now())).toList(),consumptions:(x['consumptions'] as List? ?? []).map((v)=>Consumption((v['person']??'') as String,(v['product']??'') as String,(v['quantity'] as num? ?? 0).toInt(),(v['value'] as num? ?? 0).toInt(),DateTime.tryParse(v['date']??'')??DateTime.now())).toList(),movements:(x['movements'] as List? ?? []).map((v)=>Movement((v['product']??'') as String,(v['units'] as num? ?? 0).toInt(),DateTime.tryParse(v['date']??'')??DateTime.now(),type:(v['type']??'Bodega → Freezer') as String)).toList(),expenseDetails:(x['expenseDetails'] as List? ?? []).map((v)=>Map<String,dynamic>.from(v)).toList(),sales:(x['sales'] as num? ?? 0).toInt(),expenses:(x['expenses'] as num? ?? 0).toInt(),expected:(x['expected'] as num? ?? 0).toInt(),actual:(x['actual'] as num? ?? 0).toInt(),difference:(x['difference'] as num? ?? 0).toInt());}
final ps=<Product>[Product('Toña pequeña',46,24,presentation:'cajilla',provider:'Cervecería'),Product('Litro Toña',95,12,presentation:'cajilla',provider:'Cervecería'),Product('Frost litro',80,12,presentation:'cajilla',provider:'Cervecería'),Product('Frost pequeña',40,24,presentation:'cajilla',provider:'Cervecería'),Product('Gaseosa',25,24,presentation:'cajilla',provider:'Gaseosas')];
...
class _InitialSetupState extends State<InitialSetupPage>{late final Map<String,TextEditingController> wh;late final Map<String,TextEditingController> fr;@override void initState(){super.initState();wh={for(final p in widget.products)p.name:TextEditingController()};fr={for(final p in widget.products)p.name:TextEditingController()};}@override void dispose(){for(final c in wh.values)c.dispose();for(final c in fr.values)c.dispose();super.dispose();}@override Widget build(BuildContext x)=>Scaffold(appBar:AppBar(title:const Text('Configuración inicial')),body:ListView(padding:const EdgeInsets.all(16),children:[const Text('Configura las existencias iniciales',style:TextStyle(fontSize:24,fontWeight:FontWeight.bold)),const SizedBox(height:8),const Text('Esta pantalla aparece solo la primera vez que se instala la aplicación. Registra las unidades iniciales de cada cerveza en bodega y freezer.'),const SizedBox(height:16),...widget.products.map((p)=>Card(child:Padding(padding:const EdgeInsets.all(12),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(p.name,style:const TextStyle(fontSize:18,fontWeight:FontWeight.bold)),TextField(controller:wh[p.name],keyboardType:TextInputType.number,decoration:InputDecoration(labelText:p.presentation=='cajilla'?'Cajillas iniciales en bodega':p.presentation=='paquete'?'Paquetes iniciales en bodega':'Unidades iniciales en bodega',helperText:p.presentation=='cajilla'?'Se convertirán automáticamente a unidades usando ${p.packSize} por cajilla':p.presentation=='paquete'?'Se convertirán automáticamente a unidades usando ${p.packSize} por paquete':'Ingrese unidades')),TextField(controller:fr[p.name],keyboardType:TextInputType.number,decoration:const InputDecoration(labelText:'Unidades iniciales en freezer'))])))),const SizedBox(height:12),FilledButton.icon(onPressed:()async{for(final p in widget.products){final input=int.tryParse(wh[p.name]!.text)??-1;final f=int.tryParse(fr[p.name]!.text)??-1;if(input<0||f<0)return;final w=p.presentation=='unidad'?input:input*p.packSize;p.warehouse=w;p.freezer=f;opening[p.name]=f;}await Store.save();if(mounted)widget.onSaved();},icon:const Icon(Icons.save),label:const Text('Guardar configuración inicial'))]));}
...
class PdfReport {
  static Future<void> _saveAndShare(pw.Document document, String filename) async {
  ...
  static String _money(int value) => 'C\$$value';

  static String _supplierLabel(String supplier) {
    ...
    return supplier.trim().isEmpty ? 'Sin proveedor' : supplier.trim();
  }
  ...
  static Future<void> makeMonth(List<Day> days, String month) async {
    ...
    await _saveAndShare(document, 'Inventario-Billar-$month.pdf');
  }
  ...
  static Future<void> makeQuincena(List<Day> days, String month, int q) async {
    ...
    await _saveAndShare(document, 'Inventario-Billar-$month-Q$q.pdf');
  }
  ...
  static Future<void> _addPeriod(pw.Document document, List<Day> period, String month, int q) async {
    ...
    final end = q == 1 ? DateTime(firstDate.year, firstDate.month, 16) : DateTime(firstDate.year, firstDate.month + 1, 1);
    final orders = await FeatureStore.ordersForPeriod(start, end);

    int paidValue(Map<String,dynamic> o) => int.tryParse('${o['paid'] ?? 0}') ?? 0;
    DateTime? orderDate(Map<String,dynamic> o) => DateTime.tryParse('${o['date'] ?? ''}');
    String supplier(Map<String,dynamic> o) => _supplierLabel('${o['supplier'] ?? ''}');
    final beerOrderTotal = orders.where((o) => supplier(o) == 'Cervecería').fold<int>(0,(s,o)=>s+paidValue(o));
    final gaseosasOrderTotal = orders.where((o) => supplier(o) == 'Gaseosas').fold<int>(0,(s,o)=>s+paidValue(o));
    final cigarrosOrderTotal = orders.where((o) => supplier(o) == 'Cigarros').fold<int>(0,(s,o)=>s+paidValue(o));
    final sales = sorted.fold<int>(0,(s,d)=>s+d.sales);
    final expenses = sorted.fold<int>(0,(s,d)=>s+d.expenses);
    final consumptions = sorted.fold<int>(0,(s,d)=>s+d.consumptions.fold<int>(0,(x,c)=>x+c.value));
    final loans = sorted.fold<int>(0,(s,d)=>s+d.loans.fold<int>(0,(x,l)=>x+l.amount));
    final distributable = sales - beerOrderTotal;
    ...
    final rows = sorted.map((d) {
      ...
      final dailyG=orders.where((o){final od=orderDate(o);return supplier(o)=='Gaseosas'&&od!=null&&!od.isBefore(ds)&&od.isBefore(de);}).fold<int>(0,(s,o)=>s+paidValue(o));
      final dailyC=orders.where((o){final od=orderDate(o);return supplier(o)=='Cigarros'&&od!=null&&!od.isBefore(ds)&&od.isBefore(de);}).fold<int>(0,(s,o)=>s+paidValue(o));
      return <String>[
        ...
        _money(dailyG),
        _money(dailyC),
        _money(d.consumptions.fold<int>(0,(s,c)=>s+c.value)),
        _money(d.loans.fold<int>(0,(s,l)=>s+l.amount)),
        _money(d.actual),
      ];
    }).toList();
    ...
    document.addPage(pw.MultiPage(build: (_) => [
      ...
      pw.Text('Período: ${DateFormat('dd/MM/yyyy').format(start)} al ${DateFormat('dd/MM/yyyy').format(lastDate)}'),
      pw.SizedBox(height:10),
      pw.TableHelper.fromTextArray(headers:['Fecha','Ventas','Gastos','Gaseosas','Cigarros','Consumos','Préstamos','Contado'],data:rows),
      pw.SizedBox(height:14),
      pw.Text('RESUMEN QUINCENAL',style:pw.TextStyle(fontWeight:pw.FontWeight.bold)),
      pw.Text('Ventas totales: '+_money(sales)),
      pw.Text('Pedidos Cervecería (deducción quincenal): '+_money(beerOrderTotal)),
      pw.Text('Pedidos Gaseosas (ya deducidos diariamente): '+_money(gaseosasOrderTotal)),
      pw.Text('Pedidos Cigarros (ya deducidos diariamente): '+_money(cigarrosOrderTotal)),
      pw.Text('Gastos totales: '+_money(expenses)),
      pw.Text('Consumos totales: '+_money(consumptions)),
      pw.Text('Préstamos/retiros totales: '+_money(loans)),
      pw.SizedBox(height:8),
      pw.Text('Total distribuible = Ventas - Cervecería: '+_money(distributable),style:pw.TextStyle(fontWeight:pw.FontWeight.bold)),
      pw.Text('Cada uno de los 5 socios: '+_money(share),style:pw.TextStyle(fontWeight:pw.FontWeight.bold)),
      pw.SizedBox(height:14),
      pw.Text('PEDIDOS POR PROVEEDOR',style:pw.TextStyle(fontWeight:pw.FontWeight.bold)),
      ...orders.map((o)=>pw.Text('${DateFormat('dd/MM/yyyy').format(orderDate(o)??DateTime.now())} • ${supplier(o)} • Pagado '+_money(paidValue(o)))),
      pw.SizedBox(height:14),
      pw.Text('PRÉSTAMOS / RETIROS',style:pw.TextStyle(fontWeight:pw.FontWeight.bold)),
      ...sorted.expand((d)=>d.loans.map((l)=>pw.Text('${DateFormat('dd/MM/yyyy').format(l.date)} • ${l.person} • ${l.reason} • '+_money(l.amount)))),
      pw.Text('Total préstamos: '+_money(loans)),
      pw.SizedBox(height:14),
      pw.Text('CONSUMOS POR PERSONA',style:pw.TextStyle(fontWeight:pw.FontWeight.bold)),
      ..._consumptionLines(sorted),
      pw.Text('Total consumos quincena: '+_money(consumptions)),
      pw.SizedBox(height:14),
      pw.Text('GASTOS',style:pw.TextStyle(fontWeight:pw.FontWeight.bold)),
      ...sorted.expand((d)=>d.expenseDetails.map((e)=>pw.Text('${DateFormat('dd/MM/yyyy').format(d.date)} • ${e['concept'] ?? ''} • ${e['observation'] ?? ''} • '+_money(int.tryParse('${e['amount'] ?? 0}')??0)))),
      pw.Text('Total gastos: '+_money(expenses)),
    ]));
  }

  static List<pw.Widget> _consumptionLines(List<Day> days) {
    final byPerson=<String,List<Consumption>>{};
    for(final d in days){for(final c in d.consumptions){byPerson.putIfAbsent(c.person,()=>[]).add(c);}}
    final result=<pw.Widget>[];
    for(final entry in byPerson.entries){
      final total=entry.value.fold<int>(0,(s,c)=>s+c.value);
      result.add(pw.Text(entry.key));
      for(final c in entry.value){result.add(pw.Text('  ${DateFormat('dd/MM/yyyy').format(c.date)} • ${c.quantity} x ${c.product} • '+_money(c.value)));}
      result.add(pw.Text('  Total ${entry.key}: '+_money(total),style:pw.TextStyle(fontWeight:pw.FontWeight.bold)));
    }
    return result;
  }
  ...
  static Future<void> autoGenerateIfCutoff(DateTime date, List<Day> days) async {
    ...
    int q=0;
    if(date.day==15) q=1; else if(date.day==lastDay) q=2;
    if(q==0) return;
